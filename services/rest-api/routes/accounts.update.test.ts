...
describe('services/rest-api', () => describe('routes/accounts.update', () => {
  ...
  it('should successfully update an account by ID', async () => {
    const encodedId = accountId.encodedId();
    const now = new Date();

    await request.patch(`/accounts/${encodedId}`)
      .set('Authorization', API_AUTH_HEADER)
      .send({
        data: {
          type: 'accounts',
          attributes: {
            name: 'Some other account name',
          },
        },
      })
      .expect(200)
  ...
  it('should fail to update an account by ID if it\'s not ours', async () => {
